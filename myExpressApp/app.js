  ...
  */
  ...
  for (var i = 2; i < arr.length; i++){
    ...
    if (isValidString(arr[i])) {
      ...
      // if the directory was a period is a period then that means it is only a filename
      var dir = path.dirname(oneLine[1].trim()) == "." ? "" : path.dirname(oneLine[1].trim());
      
      // get the path of the original file
      var OGBranch = path.join(source, oneLine[0].trim());

      // get the path of the destination file
      var destBranch  = path.join(dest, dir, oneLine[1].trim());
...
function checkIn(repoDir, workingDir)
{
  ...
  */
  ...
  // read the manifest file
  var manContents = fs.readFileSync(path.join(source, replaceAll(file.toString(), ',', '_') + ".rc"), 'utf-8');
  var allFiles = manContents.split('\n');
   ...
   // populate manifest file  
   populateManifest(workingDir, repoDir, allFiles, manifestPath, "checkin"); 
...
//       var out = request.query.outputMsg;
//       // request.
//       // document.getElementById("outputMsg").text("{0}").f(output_msg);
//       var output = $("#outputMsg");
//       $(output).text("{0}").f(output_msg);
//       console.log($(output).text());
//       console.log($(output).text(output_msg));
//       console.log($(output).val(output_msg));
